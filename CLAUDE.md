...
- 止盈复核（2026-08-17）：固定/追踪止盈在两新对回测中仍无净收益（回测
  ...
  不由单笔交易触发。
- 漂移守卫否决（2026-09-01）：主对 8/19-22 KR 线相对 ADS 一周重估 +992bps
  造成 −3.4% 连亏后，回测"5 日锚漂移超阈值时禁止逆漂移进场"（300/500/
  700bps，单向/双向）：全部劣于基线（7 月 +10.9% 变 −1.7%~+1.8%），且一次
  止损都没避开——止损来自进场后才形成的漂移，事前不可分辨；7 月利润恰恰
  来自逆大幅移动进场。**不做趋势/漂移过滤**。regime 事件的正确处置是人工
  停机 + 预注册重启条件（5 日 |漂移|<300bps 且溢价日均值连续 3 天走平）。
