...
**funding 套利类策略的用户风险规则：(1) perp 侧杠杆 ≤1×（开仓名义不超过保证
金），不得以更高杠杆假设做资本效率论证；(2) 紧急保证金不足严禁单腿自动减仓
——只报警由人处理，唯一可预授权的自动动作是从备用金补保证金；人工减仓须两腿
等比例同减**；全域动态回测与 A/B（对冲腿=正股 vs 对面 perp）裁决见
`extends/docs/scan/dyn_carry_backtest.md`。
